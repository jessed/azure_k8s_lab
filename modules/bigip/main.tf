...
resource "azurerm_public_ip" "mgmt_pub_ip" {
  ...
  name                            = format("${var.bigip.prefix}%02d_mgmt_pub_ip", count.index+1)
...
resource "azurerm_public_ip" "data_pub_ip" {
  count                           = var.bigip_count
  name                            = format("${var.bigip.prefix}%02d_data_pub_ip", count.index+1)
  location                        = var.rg.location
  resource_group_name             = var.rg.name
  allocation_method               = "Static"
  sku                             = "Standard"
...
resource "azurerm_network_interface" "mgmt_nic" {
  count                           = var.bigip_count
  name                            = format("${var.bigip.prefix}%02d_mgmt_nic", count.index+1)
  location                        = var.rg.location
  resource_group_name             = var.rg.name
...
resource "azurerm_network_interface" "data_nic" {
  count                           = var.bigip_count
  name                            = format("${var.bigip.prefix}%02d_data_nic", count.index+1)
  location                        = var.rg.location
  resource_group_name             = var.rg.name
...
resource "azurerm_linux_virtual_machine" "bigip" {
  ...
  computer_name                   = format("${var.bigip.prefix}%02d.${var.bigip.domain}", count.index+1)
  location                        = var.rg.location
  resource_group_name             = var.rg.name
  ...
  custom_data                     = base64encode(local_file.bigip_onboard[count.index].content)
...
/*
  ...
  identity {
    type                          = "UserAssigned"
    #identity_ids                  = [var.uai.id]
  }
*/

  source_image_reference {
    publisher                     = var.bigip.publisher
    offer                         = var.bigip.product
    sku                           = var.bigip.sku
  ...
  plan {
    publisher                     = var.bigip.publisher
    name                          = var.bigip.sku
    product                       = var.bigip.product
  }

  os_disk {
    name                          = format("${var.bigip.prefix}%02d_disk", count.index+1)
